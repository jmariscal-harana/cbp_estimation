...
%%	Load REF data (in silico or clinical) and adapt to suitable format
%	Population type
Population = 'pwdb_78_reference';
LoadFrom = 'MAT_Aortic1D';	%MAT; MAT_Aortic1D; Success; Error; PatientList;

switch LoadFrom
	case  'MAT_Aortic1D'	%Load entire population from .MAT file - formatted
		load([PATHS.REF_data,Population,'_Aortic1D.mat'],'REF')	
	
	case  'MAT'	%Load entire population from .MAT file - unformatted
		load([PATHS.REF_data,Population,'.mat'],'REF')
		for jj = 1:length(REF)
			REF_temp(jj) = ExtractData(PATHS,REF(jj),Population);
		end

	case {'Success','Error'}	%Load entire population from Nektar_Success/Nektar_Error file
		fileID = fopen([PATHS.REF_data,'Nektar_',LoadFrom,'.txt']);
		VDPatient = textscan(fileID,'%s');
		for jj = 1:length(VDPatient{1})
			REF(jj).INFO.ID = {VDPatient{1}{jj}};
			REF_temp(jj) = ExtractData(PATHS,REF(jj),Population);
		end
				
	case 'PatientList'
		% 	Load a given list of patients
		REF(1).INFO.ID = {'55art_0000-000X'};
		for jj = 1:length(REF)
			REF_temp(jj) = ExtractData(PATHS,REF(jj),Population);
		end
end
...
if exist('REF_temp') == 1
	REF = REF_temp;
	clear REF_temp
	save([PATHS.REF_data,Population,'_Aortic1D.mat'],'REF')
end
...
for jj = 1:length(REF)
	PATHS.PatientName = ['Aortic1D_',REF(jj).INFO.ID{1}];
...
function [REF] = ExtractData(PATHS,REF,Population)
...
switch Population
	case '55_art'
		...
		REF = ExtractGeometry_InSilico(PATHS,REF,Population);
...
%%		Calculate R_T, C_T
	...
	case 'pwdb_78_reference'		
		...
		%%	Extract PWV from aortic flow
		REF = ExtractPWV_InSilico(REF);
		

		%%	Extract SV, T, HR from flow data
		dt_Q					= REF.Asc_Ao.ONE_CYCLE.t(2) - REF.Asc_Ao.ONE_CYCLE.t(1);
		REF.HAEMO.SV			= dt_Q*trapz(REF.Asc_Ao.ONE_CYCLE.Q)/1e6;	%[m3]
		REF.HAEMO.T				= REF.Asc_Ao.ONE_CYCLE.t(end);
		REF.HAEMO.HR			= 60/REF.HAEMO.T;

		
		%%	Calculate tau and P_out		
		Pressure	= REF.L_Brachial.ONE_CYCLE.P*133.322312;
		t_in		= REF.L_Brachial.ONE_CYCLE.t;
		...
		%	Time where the exponential decay fitting begins
		t_fit		= REF.HAEMO.t_sys + 1/3*(REF.HAEMO.T - REF.HAEMO.t_sys);
...
%%		Calculate R_T, C_T
		REF.R_T = (mean(Pressure) - REF.P_out) / mean(REF.Asc_Ao.ONE_CYCLE.Q/1e6);	%[Pa s/m3]
		REF.C_T = tau/REF.R_T;	%[m3/Pa]
		
		if REF.R_T <= 0, error('R_T <= 0'), elseif REF.C_T <= 0, error('C_T <= 0'), end		
...
function [REF] = ExtractGeometry_InSilico(PATHS,REF,Population)
...
switch Population
	...
	case 'pwdb_78_reference'
		...
		%	This should follow the same order as the mesh connectivity 
		Artery_name			= {'Asc_Ao'; 'Ao_Arch_I'; 'Ao_Arch_II'; 'Desc_Ao_I'; 'Brachio'; 'L_Carotid'; 'L_Subclavian'};

		warning('Extrct R_in and R_out from A wave when avaialble')
		...
		warning('Temporary: writing ONE_CYCLE.A for Brachio, Carotid and Subclavian using R_in and R_oout so that we can run the whole code')
		REF.Brachio.ONE_CYCLE.A = pi*R_in(5)^2;
		REF.L_Carotid.ONE_CYCLE.A = pi*R_in(6)^2;
		REF.L_Subclavian.ONE_CYCLE.A = pi*R_in(7)^2;
...
Wave_type = 2;	%flow
SR = round(1/(REF.Asc_Ao.ONE_CYCLE.t(2)-REF.Asc_Ao.ONE_CYCLE.t(1)));
...
TT = TTAlgorithm([Wave_in';Wave_out'],SR,1,Wave_type,1,0);
