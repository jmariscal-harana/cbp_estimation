function [cv_inds, fid_pts, pulses, S_filt] = PulseAnalyse7(S, options)
...
if ~options.annotate
    try
        ...
        [fid_pts.a, fid_pts.b, fid_pts.c, fid_pts.d, fid_pts.e, fid_pts.f, fid_pts.p1, fid_pts.p2, fid_pts.s, fid_pts.dic, fid_pts.dia, fid_pts.ms] = deal(nan(length(pulses.onsets)-1,1));
...
if options.do_plot
    plot_fiducial_points(S_filt, derivs, pulses, fid_pts, no_of_pulses, options)
    plot_cv_inds(S_filt, derivs, pulses, fid_pts, cv_inds, no_of_pulses, options)
end
...
for pulse_no = 1 : length(onsets)-1
    ...
    if isempty(temp_e)
        [temp_f, temp_c, temp_d, temp_dic, temp_dia, temp_p2, temp_p1] = deal(nan);
    else
        ...
        if isempty(temp_c)
            [temp_d, temp_p2, temp_p1] = deal(nan);
        else
            % find d
            temp_d = identify_d(curr, temp_c, temp_e);
            
            % find p1
            temp_p1 = identify_p1(curr, temp_b, temp_ms, temp_s, temp_c);
            
            % find p2
            temp_p2 = identify_p2(curr, temp_d, temp_p1, temp_dic, temp_s);
...
function temp_p1 = identify_p1(curr, temp_b, temp_ms, temp_s, temp_c)
% find p1
td_pks = find_pks_trs(curr.derivs.third, 'pk');
temp = find(td_pks > temp_b,1);
temp = find(td_pks <= temp_s & td_pks <= temp_c,1,'last');
...
function temp_p2 = identify_p2(curr, temp_d, temp_p1, temp_dic, temp_s)

% % Old Approach (PulseAnalyse6):
% 
...
% New Approach (PulseAnalyse7):

% Find "p2" from the minimum value of the third derivative immediately before "d"
td_trs = find_pks_trs(curr.derivs.third, 'tr');
temp = find(td_trs >= temp_s,1,'first');
temp_p2 = td_trs(temp);
...
function plot_fiducial_points(sig, derivs, pulses, fid_pts, no_of_pulses, options)
...
function plot_cv_inds(sig, derivs, pulses, fid_pts, cv_inds, no_of_pulses, options)
...
text(mean([curr_sig.t(fid_pts.s),curr_sig.t(fid_pts.dia)]), 0.15, '\DeltaT','FontSize', fig_props.ftsize, 'Color', 'k', 'HorizontalAlignment', 'center');
% - RI
