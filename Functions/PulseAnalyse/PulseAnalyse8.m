function [cv_inds, fid_pts, pulses, sigs] = PulseAnalyse8(S, options)
...
%         
%         [fid_pts.a, fid_pts.b, fid_pts.c, fid_pts.d, fid_pts.e, fid_pts.f, fid_pts.p1, fid_pts.p2, fid_pts.p1t, fid_pts.t2t, fid_pts.s, fid_pts.dic, fid_pts.dia, fid_pts.ms] = deal(nan(length(pulses.onsets)-1,1));
...
switch no_of_pulses
    ...
    case 'single'
        ...
        derivs.first = savitzky_golay(sig.v, 1, 5);
        derivs.second = savitzky_golay(derivs.first, 1, 5);
        derivs.third = savitzky_golay(derivs.second, 1, 5);
...
% setup variables
fid_pt_names = {'a', 'b', 'c', 'd', 'e', 'f', 's', 'dia', 'dic', 'p1', 'p2', 'p1t', 'p2t', 'ms', 'f1', 'f2'};
...
for pulse_no = 1 : length(onsets)-1
    ...
    temp_p1t = temp_p1; temp_p2t = temp_p2;
    ...
    % use timings of original p1 and p2 estimates
    hold_this = temp_p1;
    temp_p1 = temp_p1t;
    temp_p1t = temp_p1;
...
% temp = find(fd_trs > temp_b & fd_trs > current_buffer,1);
temp = find(fd_trs > current_buffer,1);  % trial
temp2 = find(fd_trs > temp_b, 2); if length(temp2) > 1, temp2 = temp2(2); end
if temp2 < temp
    temp = temp2;
end
temp_p1 = fd_trs(temp);
...
% % estimate of p1, and ms. If so, take this instead
% pks = find_pks_trs(curr.sig, 'pk');
% temp = find(pks< temp_p1 & pks > temp_ms);
% if length(temp) == 1
%     temp_p1 = pks(temp);
...
%     temp_p2 = td_trs(temp+1);
    temp_p2 = td_trs(find(td_trs<temp_dic,1,'last'));  % trial
end
...
stiff_inds.t_p1_dia(good_els) = sigs.t(pts.dia(good_els)) - sigs.t(pts.p1(good_els));
stiff_inds.t_p2_dia(good_els) = sigs.t(pts.dia(good_els)) - sigs.t(pts.p2(good_els));

% calculate SIs which can be found from existing pts on pulsatile signal (amplitudes)
stiff_inds.AI(good_els) = (sigs.s(pts.p2(good_els)) - sigs.s(pts.p1(good_els)))./ (sigs.s(pts.s(good_els)) - sigs.s(pts.f1(good_els)));
stiff_inds.AP(good_els) = sigs.s(pts.p2(good_els)) - sigs.s(pts.p1(good_els));
stiff_inds.RI(good_els) = (sigs.s(pts.dia(good_els)) - sigs.s(pts.f1(good_els))) ./ (sigs.s(pts.s(good_els)) - sigs.s(pts.f1(good_els)));
stiff_inds.RI_p1(good_els) = (sigs.s(pts.dia(good_els)) - sigs.s(pts.f1(good_els))) ./ (sigs.s(pts.p1(good_els)) - sigs.s(pts.f1(good_els)));
stiff_inds.RI_p2(good_els) = (sigs.s(pts.dia(good_els)) - sigs.s(pts.f1(good_els))) ./ (sigs.s(pts.p2(good_els)) - sigs.s(pts.f1(good_els)));
stiff_inds.ratio_p2_p1(good_els) = (sigs.s(pts.p2(good_els)) - sigs.s(pts.f1(good_els))) ./ (sigs.s(pts.p1(good_els)) - sigs.s(pts.f1(good_els)));
...
if sum(good_els)>0
    stiff_inds.ms(good_els) = sigs.derivs.first(pts.ms(good_els)) ./ (sigs.s(pts.s(good_els)) - sigs.s(pts.f1(good_els)));
end
...
for pt_no = 1 : length(pt_names)
    ...
    switch curr_text
        case {'dia','p2'}
            hspace0 = 0.04;
            if (strcmp(curr_text, 'p2') || strcmp(curr_text, 'p2t')) && curr_pt.el < (fid_pts.s(beat_no) - fid_pts.f1(beat_no)+1)
                hspace0 = -0.04;
            elseif (strcmp(curr_text, 'p1') || strcmp(curr_text, 'p1t'))
                hspace0 = 0;
            end
        case 'dic'
            hspace0 = -0.01;
