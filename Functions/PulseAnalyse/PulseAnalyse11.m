function [cv_inds, fid_pts, pulses, sigs] = PulseAnalyse11(S, options)
...
sigs = calc_derivs2(S, S_filt, no_of_pulses, up, options);
...
function sigs = calc_derivs2(sig, s_filt, no_of_pulses, up, options)
...
%         derivs.f_third = savitzky_golay(derivs.f_second, 1, 9);
    ...
    case 'single'
        ...
        if ~options.do_filter
            ...
            sig.v = s_filt.v;
            % calculate derivatives of filtered signal
            derivs.first = savitzky_golay(s_filt.v, 1, 9);
...
%     curr.derivs.f_third = sigs.derivs.f_third(curr_els);
    ...
    % identify ms2
    temp_ms2 = identify_ms2(curr, temp_p1in, temp_p2pk);
...
function temp_ms2 = identify_ms2(curr, temp_p1in, temp_p2_pk)
...
for pt_no = 1 : length(pt_names)
    ...
    switch curr_text
        case {'dia','p2'}
            hspace0 = 0.04;
            if (strcmp(curr_text, 'p2pk') || strcmp(curr_text, 'p2in')) && curr_pt.el < (fid_pts.s(beat_no) - fid_pts.f1(beat_no)+1)
                hspace0 = -0.04;
