function [cv_inds, fid_pts, pulses, S_filt] = PulseAnalyse5(S, options)
...
if ~options.annotate
    ...
    fid_pts = identify_fiducial_pts(S_filt, pulses.onsets, derivs, up, no_of_pulses, options);
else
...
cv_inds = calc_stiffness_inds(S_filt, derivs, pulses, fid_pts, options);

%% Plot the fiducial points
if options.do_plot
    plot_fiducial_points(S_filt, derivs, pulses, fid_pts, no_of_pulses)
end
...
function pts = identify_fiducial_pts(sig, onsets, derivs, up, no_of_pulses, options)
...
for pulse_no = 1 : length(onsets)-1
    ...
    curr.sig = sig.v(curr_els);
    
    if strcmp(no_of_pulses, 'multiple')
        
        old.v = curr.sig; old.fs = sig.fs;
        % Eliminate low frequency content
    ...
    % find buffers
    initial_buffer = floor(buffer_tol.deb * sig.fs);  % in samples
    ...
    % find a
    temp_a = identify_a(curr, initial_buffer, temp_ms);
    
    % find b
    temp_b = identify_b(curr, temp_a);
    
    % find p1
    temp_p1 = identify_p1(curr, temp_b, temp_ms);
...
function temp_a = identify_a(curr, initial_buffer, temp_ms)

pks = find_pks_trs(curr.derivs.second, 'pk');
rel_pks = pks(pks > initial_buffer & pks < temp_ms);
...
% % Find "e" as the maximum value on the second derivative between "s" and
% % before the maximum peak on the first derivative after "s"
% lower_bound = temp_s;
% 
% pks = find_pks_trs(curr.derivs.first, 'pk');
% rel_pks = pks(pks > temp_s);
% [~, temp] = max(curr.derivs.first(rel_pks));
...
% % Find "e" as the maximum value on the second derivative between "s" and
% % before the maximum peak on the first derivative after (the min trough on the third derivative after "ms")
% lower_bound = temp_s;
% trs = find_pks_trs(curr.derivs.third, 'tr');
% rel_trs = trs(trs > temp_ms & trs < end_buffer);
% [~, temp] = min(curr.derivs.third(rel_trs));
% upper_bound = rel_trs(temp);
% if upper_bound < lower_bound || redo_log
%     if temp < length(rel_trs)
%         temp = temp+1;
%         upper_bound = rel_trs(temp);
%     end
%     if upper_bound < lower_bound && temp < length(rel_trs)
%         temp = temp+1;
%         upper_bound = rel_trs(temp);
%     end
% end
% upper_bound = min([upper_bound, 0.7*length(curr.sig)]);   % const from: https://en.wikipedia.org/wiki/QT_interval#/media/File:QT_interval_corrected_for_heart_rate.png
% 
% pks = find_pks_trs(curr.derivs.second, 'pk');
% possible_els = pks(pks >= lower_bound & pks <= upper_bound);
% 
% [~, temp] = max(curr.derivs.second(possible_els));
% if redo_log
%     if temp < length(possible_els)
%         temp = temp+1;
%     end
% end
% temp_e = possible_els(temp);
...
% % Find "e" as the minimum value on the third derivative between "s" and
% % before the maximum peak on the first derivative after (the min trough on the third derivative after "ms")
% lower_bound = temp_s;
% trs = find_pks_trs(curr.derivs.third, 'tr');
% rel_trs = trs(trs > temp_ms & trs < end_buffer);
...
%     upper_bound = rel_trs(temp+1);
% end
% 
% trs = find_pks_trs(curr.derivs.third, 'tr');
% possible_els = trs(trs >= lower_bound & trs <= upper_bound);
% 
% [~, temp] = min(curr.derivs.third(possible_els));
% temp_e = possible_els(temp);
...
% % find b (up to PulseAnalyse5)
% trs = find_pks_trs(curr.derivs.second, 'tr');
% temp = find(trs > temp_a & curr.derivs.second(trs) < 0, 1);  % added second half of criterion
% temp_b = trs(temp); clear temp

% find b (PulseAnalyse6)
trs = find_pks_trs(curr.derivs.second, 'tr');
upper_bound = 0.5*length(curr.sig);
...
% % check (in case it wasn't a trough but an inflection point)
% if temp_d == temp_b
%     trs_temp = find_pks_trs(curr.derivs.third, 'tr');
%     temp = find(trs_temp < temp_e, 1, 'last');
%     temp_d = trs_temp(temp); clear temp trs_temp
% end
...
% Identify C as the highest peak on the second derivative between "b" and "e"
pks = find_pks_trs(curr.derivs.second, 'pk');
...
% If there aren't any peaks that satisfy this criterion, then identify C as
% the first minimum on the third derivative after "b"
if isempty(temp_c)
    trs = find_pks_trs(curr.derivs.third, 'tr');
...
function temp_p1 = identify_p1(curr, temp_b, temp_ms)
% find p1
td_pks = find_pks_trs(curr.derivs.third, 'pk');
temp = find(td_pks > temp_b,1); clear d_pks
%temp = find(td_pks > temp_ms,1); clear d_pks  % This is a MODIFICATION (5th April 2018)
temp_p1 = td_pks(temp);
% % check to see if there was a maximum beforehand (rather than this being an inflection point)
% if max(curr.sig(1:temp_p1)) > curr.sig(temp_p1)
%     [~, temp] = max(curr.sig(1:temp_p1));
%     temp_p1 = temp;
% end
% clear temp td_pks
end
...
for pulse_no = start_pulse : end_pulse
    ...
    ylim([min(curr.derivs.first)-0.15*curr_range, max(curr.derivs.first)+0.15*curr_range])
    ...
    ylim([min(curr.derivs.second)-0.15*curr_range, max(curr.derivs.second)+0.15*curr_range])
    ...
    ylim([min(curr.derivs.third)-0.15*curr_range, max(curr.derivs.third)+0.15*curr_range])
...
temp = find(sig(2:end-1) > sig(1:(end-2)) & sig(2:end-1) > sig(3:end) );

trs = temp+1;

end

function stiff_inds = calc_stiffness_inds(sig, derivs, pulses, pts, options)
...
if sum(strcmp(fieldnames(sig), 'ht'))
    ht_provided = 1;
else
    ht_provided = 0;
end
...
T(good_els) = sig.t(pts.f2(good_els)) - sig.t(pts.f1(good_els));

% make vectors for each stiffness index (in case there are nans in the pts indices)
si_names = {'delta_t', 'CT', 'SI', 'CT_div_h', 'prop_s', 't_sys', 't_dia', 't_ratio', 'prop_delta_t', 't_p1_dia', 't_p2_dia', 'AI', 'AP', 'RI', 'RI_p1', 'RI_p2', 'ratio_p2_p1', 'A1', 'A2', 'IPA', 'ms', 'b_div_a', 'c_div_a', 'd_div_a', 'e_div_a', 'AGI', 'AGI_inf', 'AGI_mod', 't_b_c', 't_b_d', 'slope_b_c', 'slope_b_d', 'IPAD', 'k'};
...
stiff_inds.delta_t(good_els) = sig.t(pts.dia(good_els)) - sig.t(pts.s(good_els));
stiff_inds.CT(good_els) = sig.t(pts.s(good_els)) - sig.t(pts.f1(good_els));
if ht_provided
    stiff_inds.SI(good_els) = sig.ht./stiff_inds.delta_t(good_els);
    stiff_inds.CT_div_h(good_els) = stiff_inds.CT(good_els)./sig.ht;
end
stiff_inds.prop_s(good_els) = stiff_inds.CT(good_els)./T(good_els);
stiff_inds.t_sys(good_els) = sig.t(pts.dic(good_els)) - sig.t(pts.f1(good_els));
stiff_inds.t_dia(good_els) = sig.t(pts.f2(good_els)) - sig.t(pts.dic(good_els));
stiff_inds.t_ratio(good_els) = (sig.t(pts.s(good_els)) - sig.t(pts.f1(good_els)))./stiff_inds.t_sys(good_els);
stiff_inds.prop_delta_t(good_els) = stiff_inds.delta_t(good_els)./T(good_els);
stiff_inds.t_p1_dia(good_els) = sig.t(pts.dia(good_els)) - sig.t(pts.p1(good_els));
stiff_inds.t_p2_dia(good_els) = sig.t(pts.dia(good_els)) - sig.t(pts.p2(good_els));

% calculate SIs which can be found from existing pts on pulsatile signal (amplitudes)
stiff_inds.AI(good_els) = (sig.v(pts.p2(good_els)) - sig.v(pts.p1(good_els)))./ (sig.v(pts.s(good_els)) - sig.v(pts.f1(good_els)));
stiff_inds.AP(good_els) = sig.v(pts.p2(good_els)) - sig.v(pts.p1(good_els));
stiff_inds.RI(good_els) = (sig.v(pts.dia(good_els)) - sig.v(pts.f1(good_els))) ./ (sig.v(pts.s(good_els)) - sig.v(pts.f1(good_els)));
stiff_inds.RI_p1(good_els) = (sig.v(pts.dia(good_els)) - sig.v(pts.f1(good_els))) ./ (sig.v(pts.p1(good_els)) - sig.v(pts.f1(good_els)));
stiff_inds.RI_p2(good_els) = (sig.v(pts.dia(good_els)) - sig.v(pts.f1(good_els))) ./ (sig.v(pts.p2(good_els)) - sig.v(pts.f1(good_els)));
stiff_inds.ratio_p2_p1(good_els) = (sig.v(pts.p2(good_els)) - sig.v(pts.f1(good_els))) ./ (sig.v(pts.p1(good_els)) - sig.v(pts.f1(good_els)));
...
for beat_no = 1:length(pts.f1)
    ...
    curr_amp = sig.v(pts.s(beat_no)) - sig.v(pts.f1(beat_no));
    
    % find areas
    baseline = linspace(sig.v(pts.f1(beat_no)), sig.v(pts.f2(beat_no)), pts.f2(beat_no) - pts.f1(beat_no)+1); baseline = baseline(:);
    rel_pts = pts.f1(beat_no) : pts.dic(beat_no);
    baseline_pts = rel_pts - pts.f1(beat_no) + 1;
    stiff_inds.A1(beat_no) = sum(sig.v(rel_pts) - baseline(baseline_pts))/( sig.fs*curr_amp);
    rel_pts = pts.dic(beat_no) : pts.f2(beat_no);
    baseline_pts = rel_pts - pts.f1(beat_no) + 1;
    stiff_inds.A2(beat_no) = sum(sig.v(rel_pts) - baseline(baseline_pts))/(sig.fs*curr_amp);
...
stiff_inds.ms(good_els) = derivs.first(pts.ms(good_els)) ./ (sig.v(pts.s(good_els)) - sig.v(pts.f1(good_els)));

% Calculate SIs from second derivative (amplitudes)
stiff_inds.b_div_a(good_els) = derivs.second(pts.b(good_els)) ./ derivs.second(pts.a(good_els));
stiff_inds.c_div_a(good_els) = derivs.second(pts.c(good_els)) ./ derivs.second(pts.a(good_els));
stiff_inds.d_div_a(good_els) = derivs.second(pts.d(good_els)) ./ derivs.second(pts.a(good_els));
stiff_inds.e_div_a(good_els) = derivs.second(pts.e(good_els)) ./ derivs.second(pts.a(good_els));

stiff_inds.AGI(good_els) = ( derivs.second(pts.b(good_els)) - derivs.second(pts.c(good_els)) - derivs.second(pts.d(good_els)) - derivs.second(pts.e(good_els)) )./derivs.second(pts.a(good_els));
stiff_inds.AGI_inf(good_els) = ( derivs.second(pts.b(good_els)) - derivs.second(pts.e(good_els)) )./derivs.second(pts.a(good_els));
stiff_inds.AGI_mod(good_els) = ( derivs.second(pts.b(good_els)) - derivs.second(pts.c(good_els)) - derivs.second(pts.d(good_els)) )./derivs.second(pts.a(good_els));

stiff_inds.t_b_c(good_els) = sig.t(pts.c(good_els)) - sig.t(pts.b(good_els));
stiff_inds.t_b_d(good_els) = sig.t(pts.d(good_els)) - sig.t(pts.b(good_els));

% Calculate SIs from second derivative (slopes)
pt1.t = sig.t(pts.b(good_els));
pt1.v = derivs.second(pts.b(good_els));
pt2.t = sig.t(pts.c(good_els));
pt2.v = derivs.second(pts.c(good_els));
stiff_inds.slope_b_c(good_els) = ((pt2.v - pt1.v)./derivs.second(pts.a(good_els)))./(pt2.t-pt1.t);
pt2.t = sig.t(pts.d(good_els));
pt2.v = derivs.second(pts.d(good_els));
stiff_inds.slope_b_d(good_els) = ((pt2.v - pt1.v)./derivs.second(pts.a(good_els)))./(pt2.t-pt1.t);

% combined
stiff_inds.IPAD(good_els) = stiff_inds.IPA(good_els) + (derivs.second(pts.d(good_els)) ./ derivs.second(pts.a(good_els)));
stiff_inds.k(good_els) = (derivs.second(pts.s(good_els))) ./ (( sig.v(pts.s(good_els)) - sig.v(pts.ms(good_els)) ) ./ (sig.v(pts.s(good_els)) - sig.v(pts.f1(good_els))) );
...
function plot_fiducial_points(sig, derivs, pulses, fid_pts, no_of_pulses)
...
%- plot sig
h_b(1) = subplot('Position', [0.21,0.77,0.78,0.22]);
...
ylab = ylabel('sig', 'FontSize', ftsize, 'Rotation', 0);
set(ylab, 'Units', 'Normalized', 'Position', [-0.18, 0.5, 0]);
set(gca, 'FontSize', ftsize -2, 'YTick', 0:0.25:1)
set(gca, 'XTick', 0:0.25:1, 'XTickLabel', {})
box off

%- plot first derivative
h_b(2) = subplot('Position', [0.21,0.54,0.78,0.22]);
...
ylim([min(curr_sig.derivs.first)-0.15*curr_range, max(curr_sig.derivs.first)+0.15*curr_range])
%ylim([-0.05 0.15])
...
%- plot Second derivative
h_b(3) = subplot('Position', [0.21,0.30,0.78,0.22]);
...
ylim([min(curr_sig.derivs.second)-0.15*curr_range, max(curr_sig.derivs.second)+0.15*curr_range])
%ylim([-0.025 0.025])

% set labels
ylab = ylabel('2nd D', 'FontSize', ftsize, 'Rotation', 0);
set(ylab, 'Units', 'Normalized', 'Position', [-0.18, 0.5, 0]);
set(gca, 'FontSize', ftsize -2, 'XTick', 0:0.25:1, 'YTick', [], 'XTickLabel', {})
box off



%- plot Third derivative
h_b(4) = subplot('Position', [0.21,0.06,0.78,0.22]);
...
ylim([min(curr_sig.derivs.third)-0.15*curr_range, max(curr_sig.derivs.third)+0.15*curr_range])
%ylim([-0.025 0.025])
