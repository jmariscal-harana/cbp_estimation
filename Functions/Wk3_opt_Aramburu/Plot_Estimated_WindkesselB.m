...
function [mean_P, mean_Pest, fig_num, P_est] = Plot_Estimated_WindkesselB(t, P, Q, R1, R2, C, Pout, fig_num)
P0 = P(1);
t0 = t(1);
T  = t(end);
dt = t(2)-t(1);
...
plot(tt(1:round(1*T/dt)+1),PP(round(8*T/dt):round(9*T/dt))',tt(1:round(1*T/dt)+1),P_est(round(8*T/dt):round(9*T/dt))', 'linewidth', 2.5)
legend('P (mmHg)','P_{est} (mmHg)')
xlabel('t (s)')
ylabel('P (mmHg)')
