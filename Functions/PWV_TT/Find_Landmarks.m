...
%MARIE - additional break to exit function before while loop
if (isempty(diff2))
    return
end
...
switch show
    ...
    case 1
        figure(1)
        for i=1:2
            plot(t(indmaxs(i,:)),signal1(i,indmaxs(i,:)),'^r','MarkerSize',15,'Linewidth',1)
            plot(t(indmins(i,:,1)),indmins(i,:,2),'or','MarkerSize',15,'Linewidth',1)
            plot(t(gradins(i,:,1)),gradins(i,:,4),'*r','MarkerSize',15,'Linewidth',1)
        end
end
